...
#if 0
sectree* extractTreeHelp (std::string a) {
    int i;
    for (i = 0; i < n && isspace(a[i]); i++) {
        ;
    }
    while (isspace(a.back())) a.pop_back();
    a.erase(a.begin(), a.begin() + i);
    if (a.size() >= 4 && a.substr(0, 4) == "join") {
        a.erase(a.begin(), a.begin() + 4);
        //remove ( and ) 
        if (a.back() != ')' || a.front() != '(') {
            throw "Security level description is not well formed";
        }
        a.pop_back();
        a.erase(a.begin());
        sectree* x;
        x->level = "";
        x->left = extractTreeHelp(); //something
        x->right = extractTreeHelp(); //something
        return x;
    }
    else {
        sectree* x;
...
#if 0
std::map <std::pair <std::string, std::string>, std::string> findLCA (std::map <std::string, std::set<std::string>> &graph) {
    //precomputes all the n^2 LCA values for the given lattice
